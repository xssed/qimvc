...
class Debug {
	...
	static $msg = array (E_WARNING => '运行时警告', E_NOTICE => '运行时提醒', E_STRICT => '编码标准化警告', E_USER_ERROR => '自定义错误', E_USER_WARNING => '自定义警告', E_USER_NOTICE => '自定义提醒', 'Unkown ' => '未知错误' );
	 ...
	 *日志记录
	 */
	static function _Log($msg, $arg = '') {
		date_default_timezone_set ( 'PRC' ); //在输出时间之前设置时区，Etc/GMT为时间标准时间,PRC为北京时间
		if ($arg != '') {
			$msg = "time[" . date ( 'Y-m-d H:i:s' ) . "]\tmsg[" . $msg . "]\t    bak[" . $arg . "]\r\n";
		} else {
			$msg = "time[" . date ( 'Y-m-d H:i:s' ) . "]\tmsg[" . $msg . "]\r\n";
		}
		//echo $msg;
		return @file_put_contents ( QI_LOG . '/Log_' . date ( 'Y_m_d' ) . '.log', $msg, FILE_APPEND );
	}
