<?php
  ...
  class M_fun_pro extends DB
  {
    ...
    {
      if(stristr($id,'where '))
        $sql='select * from `fun_pro` '.$id;
      else
        $sql='select * from `fun_pro` where id='.$id;
      $cachekey = md5($sql);
    ...
    */
    ...
    {
      return D_fun_pro::Transaction_start();
    }
    /**
     * 数据库事物回滚
    */
    ...
    {
      return D_fun_pro::Transaction_rollback();
    }
    /**
     * 数据库事物提交
    */
    ...
    {
      return D_fun_pro::Transaction_commit();
    }
    /**
     * 添加数据
    */
    ...
    */
    function Delete()
    {
      return D_fun_pro::DeleteByID($this->id);
    }
  }
  class D_fun_pro extends DB
  {
    /**
     * 返回数据实体
    ...
    */
    static function GetModelByWhere($where='', $cache = 0, $cachetime = 120)
    {
      ...
      if ($cache != 0 && $GLOBALS['memcache']!=null)
        $GLOBALS['memcache']->set($cachekey, $model, false, $cachetime) or Bussiness::ErrorLog('fun_pro',0, '缓存写入失败', $sql);
      return $model;
    }
    /**
     * 返回数据实体
    ...
    */
    static function GetList($PageSize,$PageIndex,$where='',$order='', $cache = 0, $cachetime = 120)
    {
      ...
      mysql_free_result($result);
      if ($cache != 0 && $GLOBALS['memcache']!=null)
        $GLOBALS['memcache']->set($cachekey, $objects, false, $cachetime) or Bussiness::ErrorLog('fun_pro',0, '缓存写入失败', $sql);
      return $objects;
    }
    /**
     * 获取数据
    ...
    */
    ...
    {
      $sql='select '.$keys.' from `fun_pro` '.$where;
    ...
    */
    ...
    {
      $sql="delete from `fun_pro` where `id` in ($ids)";
      mysql_query($sql);
      return mysql_affected_rows();
    }
    /**
    ...
    */
    ...
    {
      return self::GetValue('count(*)','fun_pro',$where);
    }
    /**
     * 根据WHERE条件获取最大ID
    ...
    */
    ...
    {
      return self::GetValue('max(id)','fun_pro',$where);
    }
    /**
     * 数据库事物开启
    */
