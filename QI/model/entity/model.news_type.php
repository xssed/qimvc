<?php
  ...
  class M_news_type extends DB
  {
    ...
    */
    function BindForm()
    {
      ...
      if (isset($_REQUEST['isshow']))
        $this->isshow=RequestInt("isshow",0);
    ...
    */
    ...
    {
      return D_news_type::Transaction_start();
    }
    /**
     * 数据库事物回滚
    */
    ...
    {
      return D_news_type::Transaction_rollback();
    }
    /**
     * 数据库事物提交
    */
    ...
    {
      return D_news_type::Transaction_commit();
    }
    /**
     * 添加数据
    */
    ...
    */
    function Delete()
    {
      return D_news_type::DeleteByID($this->id);
    }
  }
  class D_news_type extends DB
  {
    /**
     * 返回数据实体
    ...
    */
    ...
    {
      $sql='select * from `news_type` '.$where;
      $cachekey = md5($sql);
      if ($cache == 1 && $GLOBALS['memcache']!=null) {
        if ($GLOBALS['memcache']->get($cachekey)!=false) //验证缓存
      ...
      if ($cache != 0 && $GLOBALS['memcache']!=null)
        $GLOBALS['memcache']->set($cachekey, $model, false, $cachetime) or Bussiness::ErrorLog('news_type',0, '缓存写入失败', $sql);
      return $model;
    }
    /**
     * 返回数据实体
    ...
    */
    static function GetList($PageSize,$PageIndex,$where='',$order='', $cache = 0, $cachetime = 120)
    {
      ...
      mysql_free_result($result);
      if ($cache != 0 && $GLOBALS['memcache']!=null)
        $GLOBALS['memcache']->set($cachekey, $objects, false, $cachetime) or Bussiness::ErrorLog('news_type',0, '缓存写入失败', $sql);
      return $objects;
    }
    /**
     * 获取数据
    ...
    */
    ...
    {
      $sql='select '.$keys.' from `news_type` '.$where;
    ...
    */
    ...
    {
      $sql="delete from `news_type` $where";
      mysql_query($sql);
    ...
    */
    ...
    {
      $sql="delete from `news_type` where `id` in ($ids)";
      mysql_query($sql);
      return mysql_affected_rows();
    }
    /**
    ...
    */
    ...
    {
      return self::GetValue('count(*)','news_type',$where);
    }
    /**
     * 根据WHERE条件获取最大ID
    ...
    */
    ...
    {
      return self::GetValue('max(id)','news_type',$where);
    }
    /**
     * 数据库事物开启
    */
